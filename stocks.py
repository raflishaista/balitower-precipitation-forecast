...
url = 'https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=IBM&apikey=U0PE62MA2UJ0W7LM'
r = requests.get(url)
data = r.json()


def parse_stock_data(api_response):
    """Parse Alpha Vantage TIME_SERIES_DAILY response into a DataFrame."""
...
def export_to_excel(df, filename):
    """Export DataFrame to a styled Excel workbook."""
    wb = Workbook()
    ws = wb.active
    ws.title = "Stock Data"
...
def main():
    ...
    if df is not None:
        print(df.head())
        export_to_excel(df, "outputs/stock_data.xlsx")
